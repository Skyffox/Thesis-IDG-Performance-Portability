"""Testing suite for the buffer model on the devcloud"""
...
p = Popen(["qsub -l nodes=1:iris_xe_max:quad_gpu:ppn=2 -d . build.sh"], shell=True)
p.wait()

for r in range(1, repeat + 1):
    p = Popen(["qsub -l nodes=1:iris_xe_max:quad_gpu:ppn=2 -d . rungpu.sh"], shell=True)
    p.wait()
